...
`--scaffold` (also opt-in, never part of a build) writes the empty handler file for any event member or entity-I/O verb that has none. It is write-if-absent and reports every file it writes: it never opens an existing file, never merges, never backs up. `--client-root` moves where it writes an event handler, `--server-root` where it writes an entity-I/O one.
...
`map_t::connections` is the fourth: the map's WIRING, one `connection_t` per row — see "Entity I/O" below and `entity_io_def.md`. Same reason it sits beside the entities rather than on one: no fixed count, no array-of-components in `def_gen`, and the editor's I/O tab edits a table. Carrying it across a rebuild is NOT a copy, because a row names uids: `bake_map_csg` mints fresh ones, so it remaps sender and target through what the copy actually did and drops a row whose ends did not survive.
...
- **The file format nests**, here and (since entity I/O) under `connections`, which goes two deep. `src/shared/map_blocks.{hpp,cpp}` owns the grammar: a block's members are properties *and* nested blocks, told apart by one token. A brush writes one `face` sub-block per stored surface and the map writes a `materials` block keyed by index (a block's properties are a `std::map`, so `"10"` sorts before `"2"` — the index has to be said, not implied by position). Face floats are written at `%.9g` for the same reason `brush_vertices_to_text` is: `geometry_values_equal` is bit-exact and is the undo primitive. Nothing re-keys on load — the stored plane IS the identity, and re-hulling would derive keys from the *sorted* point set the writer canonicalises to, making every reloaded brush compare unequal to itself.
...
**A SHADOW RAY IS THREE SETS, ASKED IN ORDER, and `light_occlusion_of` is what partitions
the map into them** (`transparency_plan.md` step 7, landed 2026-09-14; never looked at in a
map). The OPAQUE set decides whether the ray gets through at all; only a ray that got through
looks at the other two, because a blocked ray delivers nothing and what stands in front of the
wall it hit cannot matter. The FENCES stop it where a texel is solid and pass it whole where
one is cut away — a cutout's alpha is a coverage already resolved to yes or no, not a filter.
The GLASS stops nothing and TINTS what it passes, by `albedo · (1 − α)` per pane crossed, so
`light_visibility` answers a `vec3` where it answered a float. The unit is the whole OBJECT and
not the face, because the occluder BVH's unit is a convex piece: a brush is only as
see-through as its least see-through face, and a window is its own thin brush. One tint per
PIECE crossed, taken from the face the ray ENTERED through — the CPU steps past `t_exit` and
the kernel drops the back-facing candidate, which are the same rule spelled in the two
structures' own terms. A map with neither builds no second structure, runs no second traversal
and bakes bit-for-bit what it always did. GPU-side the three sets are three BLASes over
disjoint ranges of ONE vertex and index buffer, told apart by the instance CULL MASK with
`instanceCustomIndex` carrying each range's first triangle; the opaque geometry keeps
`VK_GEOMETRY_OPAQUE_BIT_KHR`, so the driver commits it without entering the any-hit loop and
the opaque ray is the traversal it has always been. **A CHAIN IS TINTED BY GLASS AND
LANDS ON FENCES**: `trace_nearest_surface` is what a bounce hits (opaque plus fences, a fence
counting only where its texel is solid — the same rule a shadow ray applies, or light passes
through a grate one way and not the other), and `segment_transmittance` filters the throughput
over each leg BEFORE anything is gathered at its far end. Glass is deliberately not in the
first: a chain passes THROUGH a window rather than landing on it, and reflecting off one is
`transparency_plan.md` §5's deferred Fresnel split. `resources/shaders/lightmap_shadow.glsl` is the GLSL twin, split out of
`lightmap_bake.glsl` because a shadow ray now resolves a material and so must come after the
kernel's own geometry bindings; the direct kernel gained triangles, materials and the texture
array for that one reason. What a colour cannot reach: a PROBE's four visibility channels are
the four Mixed lights and have no room beside them, so a coloured shadow dims a dynamic object
...
- **The bake rides the map PACKAGE** beside the navmesh, so a downloaded map is lit like the listen server's. Sidecar and package are version 10; an older one is REFUSED, never migrated. (`lightmap_def.md` §3)
...
**The `.def` is ONE file; the GENERATED side is many, and the split is per TYPE.** `entities_generated.hpp` and `entity_io_generated.hpp` are **umbrellas** — include either and you get everything, as before — over `generated/entities/<entity>_generated.hpp` (one per entity type: the struct, its pooled-storage invariants, and every handler it participates in, each with a comment naming the file that defines it) and `generated/traits/<trait>_generated.hpp` (one per trait: the tag type, the payload structs, the `requires` declarations and the dynamic `Entity&` overloads). Two **cores** sit under both — `entities_core_generated.hpp` (enums, `enum_traits`, components, base `Entity`) and `entity_io_core_generated.hpp` (the three derived enums, the `input_context_t` forward declaration) — because a per-type header cannot include its own umbrella, so the dependency has to point the other way. `entities_generated.cpp` and `entity_io_generated.cpp` each stay ONE file, since every table in them spans all types. **The input stays one file on purpose**: an entity already leans on declarations elsewhere in the `.def` (its components, its enums, the base, now a trait), so splitting the input would turn each of those lookups into a file open and make `entity_type`'s order the order files are listed. The RESOLVED picture is what belongs per type.

The per-entity and per-trait headers are deliberately **not** listed as custom-command outputs — that list would be a hand-maintained mirror of the `.def`, the drift the asset pipeline's one-walk rule deleted. `def_gen` creates the two directories and never cleans them, so a renamed type leaves an old header behind: visible in `git status`, included by nothing.
...
- Per-type behavior is a handwritten **exhaustive switch** over the closed enum (`create_map_entity`, `collect_movement_volumes`, `draw_entity_hitbox_overlay`). That's the sanctioned pattern; adding an entity makes each switch a compile error, which is the point. **Storage is not on that list** — `Entity_System` sizes one byte pool per tag from `ENTITY_INFOS` directly, so a new entity needs no case anywhere in it (`make_entity_pool` was the fifth switch and is gone; see `entity_system_def.md`).

**Iterating a trait: `entities_with_trait<Trait_T>()`.** Three views over `Entity_System`, and the one to reach for is decided by what the question IS:

- `entities_of<T>()` — one pool. For code that needs T's own fields.
- `entities_with<Component_T...>()` — every pool whose type embeds all of them, the row carrying each component by reference.
- `entities_with_trait<Trait_T>()` — every pool whose type carries the trait, the row carrying the components that trait's **`requires`** names. A trait with no `requires` yields the `Entity&` bare, so the loop is `for (entities::Entity& e : ...)` rather than a one-element structured binding.

**Ask what the `.def` DECLARES, not what the layout happens to say.** "Who is `Mortal`" and "who carries `Health`" name the same pools today and are not the same question — `is Mortal` and `health: Health` are two separate lines, so a type can gain the component without opting into the trait that reads it. `prediction_def.md` §5.1 is the argument and the worked conversions (the trigger volumes, the hitscan targets).

All three share ONE iterator, `Pool_View<Pool_Filter_T, Component_T...>`, whose filter is a policy — a `component_mask` test or an `ENTITY_TRAIT_MASKS` test, constant per pool and evaluated once per pool rather than once per entity. That is the whole reason they are not three loops: `for (pool) for (slot) if (get_render(e))` paid a `COMPONENT_OFFSETS` lookup per entity that could only ever return -1 for most of them.

**`requires` is the tool for a component that must be there.** A view over a trait with no `requires` hands back only the `Entity&`, so the loop has to `get_component<C>()` and skip a null — a silent skip, and one whose null branch is typically dead by coincidence rather than by design. Adding the component to the trait's `requires` makes it non-null in the row and makes a type that lacks it a **generator error**. That is what `Touchable :: trait requires Box_Volume, Enabled` bought, and it cost nothing at connect: `mix_schema_hash` deliberately covers neither a trait's `requires`, its `by`, nor an entity's `is` list, none of which reach the wire. The counter-case is `shared::light_is_switched_on`, whose "no `Enabled` means always on" branch is genuinely LIVE (`Directional_Light_Entity` is not `is Switchable`) — so it stays a rule rather than becoming a `requires`.
...
- **`collect_touchers` walks exactly what `by` names**, players and physics bodies, and a `static_assert` against `SIGNAL_ACTIVATOR_MASKS[Touched]` is what keeps the two in step — a type that can touch and is not walked here passes a check nothing then honours, and now stops the build instead. It stays two hand-written arms on purpose: a toucher's BOUNDS are per type and come off no component, while the volumes it tests against are `entities_with_trait<Touchable>()` (see "Iterating a trait" below). A crate could never fire anything under the loop this replaced, which is one of the three defects `entity_io_def.md` §2 names. Both bounds are what the thing physically IS, never `compute_entity_bounds`, which answers with the drawn mesh: the player's movement hull, and a body's own `size` half-extents. That also keeps the asset system out of a function with no other reason to need it.
...
Collision geometry is deliberately NOT in that list — brushes and static meshes are map-owned values (see "Map vs Session"). A `Box_Volume` is carried by the three volume types (trigger, jump pad, reflection volume) and by `Damageable_Entity`, whose box is its HITBOX -- a component rather than a bare v3 so the editor's reshape gizmo and the pick treat it as it treats a trigger's, and so it can sit off the origin. It carries the component without `is Touchable`, which is the trait-versus-component split doing its job.
...
**The hand-written halves are named by what they FILL, and they are not in `generated/`.** A binder TU references symbols nobody has written yet, so the seam is a link error and the counterpart file is where that error is answered: `src/client/effects/<member>.cpp` and `src/client/game_events/<member>.cpp` for a channel member, and for an entity-I/O action **the file where its RECEIVER's type is declared** — `src/server/traits/<trait>.cpp` when the trait has `requires` (`switchable.cpp`, `mortal.cpp`: one copy of the verb against the component, for every type that opts in), `src/server/entities/<entity>.cpp` when it does not (`point_light_entity.cpp`, `spot_light_entity.cpp`: this type's own answer). An entity with no per-type handler gets no file. `--scaffold` writes all of them once, write-if-absent, with `--client-root` / `--server-root` saying where.

**The stale direction has its own guard, and it is `-Werror=missing-prototypes`.** The link error covers "trait added, handler missing"; "trait removed, handler still defined" used to compile silently as dead code. Each handler TU includes the ONE generated header that declares its handlers, so a definition whose declaration went away has no prior prototype and is an error naming it. Set per-source on `ENTITY_IO_HANDLERS` in CMakeLists.txt — which is why that list is named there rather than written inline in the `game_server` sources — and it is why a helper in one of those files must sit in an anonymous namespace. Under Clang this is **not** `-Wmissing-declarations`, which is about a declaration that declares nothing.
...
**How the UI binds to game state: STRUCTURE IS RETAINED, VALUES ARE REWRITTEN.** Every node property has exactly one of three owners — **authored** (labels, the parent/child wiring; written once by the build), **bound** (rewritten *every frame* from a source outside the node, never cached), **animated** (opacity, offsets; advanced by `dt`). There is no `hud_health` and no `set_health`, there is `try_find_my_player(ctx)->health` read where it is drawn. Push and observer bindings both buy a second copy that can disagree, the failure `body_yaw`, `held_snapshot_tick` and `last_broadcast_cvars` each already paid for; signals/slots additionally fit this data badly, since snapshots replace state wholesale and there is no "changed" moment to emit.
...
**How the editor treats an entity is TWO halves, and they are different kinds of fact** (`editor/entity_editor_traits.{hpp,cpp}`). The PER-TYPE half is `editor_data_per_entity_type_t`: colour, icon, stand-in shape and the three draw functions (stand-in, diagram, reach), a `constexpr Enum_Array` keyed by `entity_type` and pinned by `rows_in_enum_order` -- a table of constants, never a switch rebuilt per call. The PER-INSTANCE half is `editor_shape_at(entity, position)`, the ONE answer to "what shape is this entity drawn and picked as", by one ordered rule with no type arm but the stand-in: a `Box_Volume` component, else a visible `Render` mesh under its scale, else the type's stand-in (the player hull, the spectate frustum, the pyramid marker), else a small point box so it can be clicked at all. The picking BVH, the placement ghost, the selection highlight, framing and box select all read that one function (`editor_bounds_of`, `editor_collision_planes_of`, `editor_object_bounds.hpp`), so the box you click IS the box you see -- before the split the ghost and the pick were two hand-written switches that disagreed for spectate spots, players, physics bodies and lights. It returns a SHAPE, not an AABB, because the spectate frustum's corner is empty space and a click there must fall through. Placement lifts by however far the shape reaches below its origin, so feet-versus-centred is implicit in the shape rather than a row field. `shared::compute_object_bounds` is a DIFFERENT question -- the extent an entity's components spell out, with no stand-in -- and is what a prefab's anchor and a pasted piece's low corner use; there is deliberately no `compute_entity_bounds` in shared any more, since nothing gameplay-side ever asked it. What a draw needs from outside the entity (the jump pad arc's gravity) rides `editor_context_t::entity_draw_settings`, cut from the cvars once per frame.
...
- **Ranges: three house types, and only three.** `Span<T>` (`shared/span.hpp`) is the one non-owning view — it replaces every pointer-plus-count spelling. `Array<T, N>` and `Enum_Array<Enum_T, T>` (`shared/array.hpp`) are the owning fixed-size pair; both are aggregates that convert implicitly to `Span`, and both are trivially copyable exactly when `T` is, so one can sit in an entity struct without breaking the blittable contract. Prefer them to `std::array` in new code; there is no dynamic house array (`std::vector` stays).
  - `Enum_Array` is sized from `enum_traits<Enum_T>::count`, which `def_gen` emits per enum in every `.def` (generated ones also carry the enum's `enum_type` id — the compile-time link to its runtime reflection tag). A hand-written enum specializes `enum_traits` next to itself with `count` alone, and so do the **DERIVED** enums `def_gen` emits from no declaration — `entity_type`, `component_type`, `enum_type`, `entity_action`, `entity_signal`, `entity_trait` — which have no `enum_type` id to carry, there being no `ENUM_INFOS` row for them. They are dense from 0 like any other, so an `Enum_Array` over one is legal; without the specialization a caller silently reaches for a raw `Array` sized by the `_COUNT`, which is a table that stops resizing when the enum grows. `entity_type::Invalid` is slot 0 of any table over it.
