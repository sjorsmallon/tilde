...
# Run the whole test suite (~2s, all 39)
...
**Lightmaps do not exist yet and the design must not close the door on them.** Four things follow, all in `geometry_def.md` §10: a face's UV parameterization is the **material channel** (`face_uv_channel_t`, a named type) because a lightmap needs a second, unique, atlas-packed channel that is derived rather than authored; `lightmap_scale` and `smoothing_group` are carried with no consumer *on purpose*, because they are AUTHORED data in files — adding a runtime field later is free, adding an authored one means migrating every map or silently defaulting it; baked output belongs in a **sidecar** beside the `.source` (the `.navmesh` precedent), never in `map_t`; and the second UV is a **parallel array** (`std::vector<vec2f> lightmap_uv`, `empty()` being the whole test), never a widened vertex — `skeleton.hpp` already made exactly this call for skin weights and the reasoning transfers unchanged. `vertex_xnuu` in `shared/vertex.hpp` was dead and is deleted; the parallel arrays beside it (`skin`, `blend`) are the pattern a lightmap UV follows. Convex decomposition is collision-only for the same family of reasons — splitting render faces along collision seams would fragment charts.
