...
static void create_swapchain() {
  ...
  VkPresentModeKHR present_mode = VK_PRESENT_MODE_FIFO_KHR; // V-Sync
  // Optional: Allow IMMEDIATE (no vsync) or MAILBOX
...
bool Init() {
  ...
  g_physical_device = devices[0]; // Just pick the first one
  ...
  // Descriptor Pool (Required for ImGui)
  ...
  // Upload fonts
  // Use a temporary command buffer for font upload
  {
    ...
    ImGui_ImplVulkan_CreateFontsTexture(); // Old versions passed command
                                           // buffer, newer ones don't
    ...
    // ImGui_ImplVulkan_DestroyFontUploadObjects(); // Only for older versions
    // of imgui?
  }
...
bool Tick() {
  ...
  render_pass_info.framebuffer = g_swapchain_framebuffers[image_index];
