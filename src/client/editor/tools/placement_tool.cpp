  ...
  static const std::vector<placeable_t> table = [] {
    ...
    for (entities::entity_type type : entities::placeable_entity_types())
      result.push_back({entities::entity_info(type).display_name, false,
                        shared::geometry_kind_t::Brush, type});
...
void Placement_Tool::on_enable(editor_context_t& ctx)
{
  ...
  // Default to a box brush.
  if (!geometry_to_place && !entity_to_place) select_placeable(get_index_of_placeable("BRUSH"));
...
{
  if (e.button != input::mouse_button_t::Left || !cursor_is_currently_over_surface) return;
...
void Placement_Tool::on_mouse_drag(editor_context_t& ctx, const input::mouse_event_t& e) {}
void Placement_Tool::on_mouse_up(editor_context_t& ctx, const input::mouse_event_t& e){}

static shared::geometry_value_t make_placement_prototype(shared::geometry_kind_t kind)
{
  ...
  log_error("make_placement_prototype: unhandled geometry kind {} — using a brush", (int)kind);
  return shared::make_box_brush({0, 0, 0}, default_extents);
}
