...
struct editor_context_t
{
  ...
  float work_plane_height         = 0.0f;
  bool  placement_prefers_surface = false;
  ...
  // The group a pasted `requested_paste` becomes, named after the prefab it
...
try_pick_work_plane_point(const editor_context_t& ctx, const viewport_state_t& view)
{
  const float step = ctx.grid ? ctx.grid->step() : editor::MAJOR_GRID_STEP;

  const linalg::vec3 plane_point{0, ctx.work_plane_height, 0};
  ...
  const linalg::vec3 point = view.mouse_ray.origin + view.mouse_ray.direction * t;
  return linalg::vec3{editor::snap(point.x, step), ctx.work_plane_height,
                      editor::snap(point.z, step)};
}

[[nodiscard]] inline std::optional<linalg::vec3>
try_pick_surface_point(const editor_context_t& ctx, const viewport_state_t& view)
{
  ...
  const float        step  = ctx.grid ? ctx.grid->step() : editor::MAJOR_GRID_STEP;
  const linalg::vec3 point = view.mouse_ray.origin + view.mouse_ray.direction * hit.t;
...
{
  if (ctx.placement_prefers_surface)
  {
    if (const std::optional<linalg::vec3> surface = try_pick_surface_point(ctx, view))
      return surface;
    return try_pick_work_plane_point(ctx, view);
  }

  if (const std::optional<linalg::vec3> plane = try_pick_work_plane_point(ctx, view))
    return plane;
  return try_pick_surface_point(ctx, view);
}
