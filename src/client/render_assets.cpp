...
std::unordered_map<std::string, renderer::texture_handle_t> g_texture_by_path;
...
{
  const auto cached = g_texture_by_path.find(path);
  if (cached != g_texture_by_path.end())
    return cached->second;

  const assets::asset_handle_t<assets::texture_asset_t> asset = assets::load_texture(path);
  const assets::texture_asset_t *texture = asset.valid() ? assets::get(asset) : nullptr;

  renderer::texture_handle_t handle;
  if (texture)
    handle = renderer::register_texture(*texture, /*srgb*/ true);
  else
    log_error("[render_assets] could not load texture '{}'", path);

  g_texture_by_path[path] = handle;
  return handle;
}
