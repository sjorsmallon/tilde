...
static_assert(std::is_trivially_copyable_v<Jump_Pad_Entity>,
              "Jump_Pad_Entity must stay trivially copyable: pooled storage, snapshot "
              "baselines and undo all copy entities with memcpy");
static_assert(std::is_trivially_destructible_v<Jump_Pad_Entity>,
              "Jump_Pad_Entity must stay trivially destructible: the entity pool frees a "
              "slot by overwriting it and runs no destructor");
static_assert(std::is_base_of_v<Entity, Jump_Pad_Entity>,
              "Jump_Pad_Entity must derive from Entity: the generated tables hand out "
              "Entity* for every entity type");

// --- what a Jump_Pad_Entity accepts ---
//
...
// --- what a Jump_Pad_Entity announces ---
//
