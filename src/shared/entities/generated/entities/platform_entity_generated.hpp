...
static_assert(std::is_trivially_copyable_v<Platform_Entity>,
              "Platform_Entity must stay trivially copyable: pooled storage, snapshot "
              "baselines and undo all copy entities with memcpy");
static_assert(std::is_trivially_destructible_v<Platform_Entity>,
              "Platform_Entity must stay trivially destructible: the entity pool frees a "
              "slot by overwriting it and runs no destructor");
static_assert(std::is_base_of_v<Entity, Platform_Entity>,
              "Platform_Entity must derive from Entity: the generated tables hand out "
