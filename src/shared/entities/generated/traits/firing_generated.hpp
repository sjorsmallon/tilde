...
//
// The trait Firing: every verb it declares, and the handler shape those
...
// A tag type, so `is<Firing>(e)` is one name rather than a value and a
// template argument that could disagree.
struct Firing
{
  static constexpr entity_trait tag = entity_trait::Firing;
...
static_assert(std::is_trivially_copyable_v<Fire_Data>,
...
void fire(Entity&, const Fire_Data&, input_context_t&);
[[nodiscard]] bool try_fire(Entity&, const Fire_Data&, input_context_t&);
