...
// The falloff, the cone, the type tag AND the light struct are
// light_arrival.glsl's, which is the game's and the bake's too -- this header
// held a FOURTH copy of the maths and a SECOND copy of the struct, and its
...
// 1/PI is the same one shade_direct and lightmap_residual_diffuse apply, for the same
...
vec3 compute_lighting(vec3 world_position, vec3 world_normal) {
    ...
    for (int i = 0; i < scene.light_count && i < MAX_LIGHTS; i++) {
        ...
        float n_dot_l = max(dot(world_normal, arrival.direction), 0.0);
        result += (light.radiance.rgb / PI) * n_dot_l * arrival.attenuation;
    }
